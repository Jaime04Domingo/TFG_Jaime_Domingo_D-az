...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 11 18:57:22 2026

@author: jaime
"""

"""
MODELO 1 MEJORADO — Regresión Lineal con Dummies de Mes
...
print("=" * 68)
print("  MODELO 1 MEJORADO — REGRESIÓN CON DUMMIES DE MES")
print("=" * 68)
...
# Variables base (sin mes numérico)
FEATURES_BASE = [
    "trafico_medio",
    "T2M_MAX",
    "T2M_MIN",
...
MESES_ES = {
    ...
    # Diciembre es la categoría de referencia (se omite para evitar multicolinealidad perfecta)
}

# Crear dummies de mes (referencia = diciembre = mes 12)
...
# Variance Inflation Factor: VIF = 1/(1-R²_j)
# donde R²_j es el R² de regresión de variable j contra todas las demás
# VIF < 5: sin problema | 5-10: moderada | > 10: alta
# ═══════════════════════════════════════════════════════════════════════
print("\n" + "="*68)
...
# Calcular VIF sobre los datos de entrenamiento
...
if not problemas_vif:
    print(f"\n  ✅ Sin problemas de multicolinealidad. Todos los VIF < 10.")
else:
...
# ═══════════════════════════════════════════════════════════════════════
print("\n" + "="*68)
print("  MODELO CON DUMMIES DE MES")
print("="*68)
...
def rmse(yr, yp): return float(np.sqrt(np.mean((yr-yp)**2)))
def mae(yr, yp):  return float(np.mean(np.abs(yr-yp)))
...
def estandarizar(arr): return (arr - arr.mean()) / (arr.std() + 1e-10)
...
for i, col in enumerate(FEATURES_BASE):
    ...
    if col == "trafico_medio":
        ...
        interp = f"→ FdS reduce {abs(c):.3f} µg/m³"
    elif col == "anyo":
...
# ═══════════════════════════════════════════════════════════════════════
print("\n  COMPARATIVA CON VERSIÓN ANTERIOR (mes como numérico):")
print(f"  R² test anterior   : 0.6849")
print(f"  R² test con dummies: {r2_test:.4f}  ({r2_test-0.6849:+.4f})")
print(f"  RMSE test anterior : 7.454 µg/m³")
print(f"  RMSE test dummies  : {rmse_test:.3f} µg/m³  ({rmse_test-7.454:+.3f})")
...
colores[-1] = "#90A4AE"  # gris para referencia
...
ax.set_title("Efecto de cada mes sobre el NO₂\n"
             "Controlando por tráfico, clima y año · Referencia = Diciembre",
             fontsize=12, fontweight="bold")

leg = [mpatches.Patch(color="#E53935", label="Más NO₂ que diciembre"),
...
plt.tight_layout()
plt.savefig(CARPETA + "A09_coeficientes_dummies_mes.png", dpi=150, bbox_inches="tight")
plt.close()
print("  ✓ A09_coeficientes_dummies_mes.png")

# G2: VIF por variable
print("[G2] VIF por variable...")
nombres_vif = [NOMBRES.get(f, f) for f in FEATURES]
vifs_plot   = [min(v, 50) for v in vifs]  # capping para visualización
cols_vif    = ["#E53935" if v > 10 else "#FB8C00" if v > 5 else "#43A047"
               for v in vifs]
...
for bar, val in zip(bars, vifs):
    txt = f"{val:.1f}" if val < 50 else f"{val:.1f}"
    ax.text(float(bar.get_width()) + 0.2,
            float(bar.get_y()) + float(bar.get_height())/2,
            txt, va="center", fontsize=7.5, fontweight="bold")

ax.set_xlabel("Factor de Inflación de la Varianza (VIF)", fontsize=11)
ax.set_title("Verificación de multicolinealidad — VIF por variable\n"
             "Verde <5: sin problema · Naranja 5-10: moderado · Rojo >10: alto",
             fontsize=12, fontweight="bold")
ax.legend(fontsize=9)

plt.tight_layout()
plt.savefig(CARPETA + "A10_VIF_multicolinealidad.png", dpi=150, bbox_inches="tight")
plt.close()
print("  ✓ A10_VIF_multicolinealidad.png")

# G3: Predicciones vs real (modelo con dummies)
print("[G3] Predicciones vs real...")
fechas_test = df["fecha"].iloc[n_train:n_train+n_test].values
res_test    = y_test - y_pred_test

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 9), sharex=True)
fig.suptitle("Modelo 1 con dummies de mes — Predicciones vs valores reales\n"
...
ax1.plot(fechas_test, y_pred_test,   color="#1E88E5", linewidth=1.2, linestyle="--",
         alpha=0.8, label="NO₂ predicho (dummies mes)")
...
ax2.set_title(f"Residuos  |  Media={float(np.mean(res_test)):.2f} µg/m³  Sesgo={float(np.mean(res_test)):.2f}",
              fontsize=10, fontweight="bold")

plt.tight_layout()
plt.savefig(CARPETA + "A11_predicciones_dummies_mes.png", dpi=150, bbox_inches="tight")
plt.close()
print("  ✓ A11_predicciones_dummies_mes.png")

# ── RESUMEN ───────────────────────────────────────────────────────────
print("\n" + "="*68)
print("  RESUMEN FINAL")
print("="*68)
print(f"\n  Modelo con dummies de mes vs versión con mes numérico:")
print(f"  R²   test: 0.6849 → {r2_test:.4f}  ({r2_test-0.6849:+.4f})")
print(f"  RMSE test: 7.454  → {rmse_test:.3f}  ({rmse_test-7.454:+.3f} µg/m³)")
print(f"  MAE  test: 5.984  → {mae_test:.3f}  ({mae_test-5.984:+.3f} µg/m³)")
print(f"\n  Multicolinealidad: {'✅ Sin problemas' if not problemas_vif else f'⚠ {problemas_vif}'}")
...
print(f"\n  Gráficos:")
print(f"    A09_coeficientes_dummies_mes.png")
print(f"    A10_VIF_multicolinealidad.png")
print(f"    A11_predicciones_dummies_mes.png")
